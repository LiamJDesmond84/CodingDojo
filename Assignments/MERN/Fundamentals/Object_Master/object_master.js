...
const fire = pokémon.filter( p => p.types[0] === "fire" );
const grthan1 = pokémon.filter( p => p.types[1]);
const names = pokémon.map( p => p.name )
const over69 = pokémon.filter(p => p.id > 99).map( p => p.name )
const onlypoison = pokémon.filter(p => p.types == "poison").map( p => p.name)
const secondflying = pokémon.filter(p => p.types[1] == "flying").map( p => p.name)[0]
